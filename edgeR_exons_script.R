...
fcc <- read.table("tolerance.counts.meta.fc",stringsAsFactors = FALSE,header = TRUE)
...
keep <- rowSums(cpm(fcc3))>8
fcc3 <- fcc3[keep,,keep.lib.sizes=FALSE]
dim(fcc3)
hist(log(rowSums(fcc3$counts),base=10),xlim=c(0,5))
...
image(cpm(fcc3,log=TRUE,prior.count=1)[order(rowSums(fcc3$counts),decreasing = T),])
...
lrt <- glmLRT(fit, contrast=cont.matrix[,1])
topTags(lrt)
...
for(i in 1:ncont){
	tmp <- topTags(lrt[[i]],sort.by="none",n=length(fcc3$counts[,1]))
	dim(tmp$table)
...
subi <- !grepl("9999",colnames(fcc3$counts))
subi <- c(grep("F1",colnames(fcc3$counts)),grep("F2",colnames(fcc3$counts)))
#subg <- which(outvals[["FDR"]][,7]<0.05&rowSums(outvals[["FDR"]]<0.05)==1)
subg <- which(outvals[["FDR"]][,1]>0.05&outvals[["FDR"]][,1]<0.15)
subg <- grep("Funhe2EKm036752|Funhe2EKm036757t1",fcc3$genes[,1]) #ahr 10074
subg <- grep("Funhe2EKm005886|Funhe2EKm005887",fcc3$genes[,1]) #ahr 217

subg <- grep("Funhe2EKm026004t1",fcc3$genes[,1])
subg <- fcc3$genes[,2]=="Scaffold9893"&fcc3$genes[,3]>1.8e+06&fcc3$genes[,3]<2.05e+06
#subg <- which((outvals[["FDR"]][,1]<0.05)&(outvals[["FDR"]][,3]>0.05))

plotMDS(subC[subg,subi],
...
##no good?
sps <- spliceVariants(fcc3,geneID=fcc3$genes[,1])

##diffSpliceDGE. Where did this function COME FROM? GOOGLING YIELDS NOTHING
dsfit<-diffSpliceDGE(fit, coef=1, geneid=paste(fcc3$genes[,1],fcc3$genes[,2]), exonid=NULL, verbose=TRUE)
