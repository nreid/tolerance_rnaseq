...
keep <- rowSums(cpm(fcc3))>10
fcc3 <- fcc3[keep,,keep.lib.sizes=FALSE]
dim(fcc3)

##keep only genes falling in "outlier" regions
roi<-read.table("~/tolerance_rnaseq/roi_genes.txt",stringsAsFactors=FALSE)
roi<-cbind(roi,gsub("Parent=","",roi[,3]))
roi<-cbind(roi,gsub(";","",roi[,4]))
